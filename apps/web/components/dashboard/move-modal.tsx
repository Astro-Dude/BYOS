  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-lg bg-white dark:bg-zinc-900 p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <h3 className="text-base font-semibold text-zinc-900 dark:text-zinc-100">Move file</h3>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-1 text-sm">
          ...
          {crumbs.map((c, i) => (
            <span key={c.id} className="flex items-center gap-1">
              ...
              <button
                onClick={() => jumpTo(i)}
                className={i === crumbs.length - 1 ? "text-zinc-700 dark:text-zinc-300" : "text-indigo-600 hover:underline"}
              >
                {c.name}
              </button>
            </span>
        ...
        <div className="mt-3 max-h-60 overflow-auto rounded-lg border border-zinc-200 dark:border-zinc-800">
          {folders.length === 0 ? (
            ...
            folders.map((f) => (
              <button
                key={f.id}
                onClick={() => enter(f)}
                className="flex w-full items-center justify-between gap-2 border-b border-zinc-50 px-3 py-2 text-left text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800"
              >
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <span className="truncate text-xs text-zinc-500">
            Destination: <span className="font-medium text-zinc-700 dark:text-zinc-300">{destName}</span>
          </span>
          <div className="flex gap-2">
            <Button onClick={onClose} className="bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-200">
              Cancel
            </Button>
